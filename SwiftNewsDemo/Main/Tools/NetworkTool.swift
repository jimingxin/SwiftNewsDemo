...
protocol NetworkToolProtocal {
    ...
    // 获取首页顶部标题内容
    static func loadHomeTitlesData(completionHsndler:@escaping (_ topTitles: [TopicTitle]) ->())
    ...
    static func loadHomeCategoryNewsFeed(category: String, completionHsndler: @escaping (_ nowTime: TimeInterval, _ newsTopics: [WeiTouTiao]) -> ())
...
class NetworkTool: NetworkToolProtocal {
    ...
    /// 获取首页顶部标题内容
    class func loadHomeTitlesData(completionHsndler completionHandler:@escaping (_ topTitles: [TopicTitle])->()) {
        let url = BASE_URL + "article/category/get_subscribed/v1/?"
        let params = ["device_id": device_id,
                      "aid": 13,
                      "iid": IID] as [String : AnyObject]
    ...
    /// 获取首页不同分类的新闻内容(和视频内容使用一个接口)
    class func loadHomeCategoryNewsFeed(category: String, completionHsndler completionHandler:@escaping (_ nowTime: TimeInterval,_ newsTopics: [WeiTouTiao])->()) {
        ...
        Alamofire.request(articleURL).responseString { (response) in
            ...
            if let value = response.result.value {
                ...
                if value.contains("BASE_DATA.galleryInfo =") { // 则是图文详情
                    ...
                    let sub_abstracts = value[titleRange]
